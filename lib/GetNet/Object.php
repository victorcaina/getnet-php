...
class GetNet_Object implements ArrayAccess, Iterator
{
	...
	public function __construct($response = array())
	{
		...
		$this->_unsavedAttributes = new GetNet_Set();
		$this->_position = 0;
	...
	{
		$var = GetNet_Util::fromCamelCase(substr($name,3));
	...
	public function unsavedArray()
	{
		...
		foreach($this->_unsavedAttributes->toArray() as $a) {
			if($this->_attributes[$a] instanceof GetNet_Object) {
	...
	public function refresh($response)
	{
		...
		foreach($response as $key => $value) {
			$this->_attributes[$key] = GetNet_Util::convertToGetNetObject($value);
			$this->_unsavedAttributes->remove($key);
		}
	...
	{
		if ($recursive) {
			return GetNet_Util::convertGetNetObjectToArray($this->_attributes);
		}else {
			return $this->_attributes;
		}
	}
